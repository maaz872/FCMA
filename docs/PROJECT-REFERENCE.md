...
│   └── seed.ts                    # 1807-line seed (admin, demo user, 5 demo clients, etc.)
├── public/
...
### `WorkoutCategory` / `WorkoutSubcategory` / `Workout` (live rows: 30 / 168 / 4)

Same multi-tenant pattern. `Workout` carries `title`, `slug`, `description`, `videoUrl`, `instructions` (stringified JSON array), `subcategoryId`, `difficulty @default("Intermediate")`, `duration?`, `sets?`, `reps?`, `targetGoal?`, `isPublished`. Indexed on `[subcategoryId, isPublished]`.

Default seed: 5 workout categories with 28 subcategories total.
...
### `PlanTemplate` (live rows: 7) / `PlanTemplateDay` (209) / `ClientPlan` (5) / `ClientPlanDay` (170) / `PlanDayMeal` (970) / `DailyProgress` (3)
...
| GET / POST | `/api/admin/workouts` | `requireCoach()` | Both safe except subcategoryId FK isn't checked (Gotcha #16). |
| GET / PUT / DELETE | `/api/admin/workouts/[id]` | `requireCoach()` + `findFirst({coachId})` | Same FK concern on PUT. |
...
| GET / POST | `/api/admin/plans/[id]/days` | `requireCoach()` + template ownership | POST is destructive: `deleteMany` template days then bulk-insert. **FKs not cross-coach-checked.** |
...
| GET | `/api/user/plan` | `getCurrentUser()` | Returns current/selected day. |
...
| `/hub/my-plan` | `my-plan/page.tsx` | `fetchWithRetry` GET `/api/user/plan` + `/api/user/targets`; on date change GET `/api/user/plan?date=`; off-plan search GET `/api/food-items?search=` (debounced 300ms) | POST `/api/user/plan/progress` (checkboxes), POST `/api/meals` (off-plan logging) | Biggest page (~800 lines); empty-state if no plan; past/future days view-only. |
...
| `/hub/workouts` | `workouts/page.tsx` (server) + `WorkoutsBrowser.tsx` (client) | Server: Prisma workout+categories by coachId | none | Sidebar filters (search/cat/subcat/difficulty/goal). No pagination. |
| `/hub/workouts/[slug]` | `[slug]/page.tsx` | Server: Prisma findFirst+3 related workouts | none | `notFound()` on miss. |
...
| `/admin/plans/[id]/edit` | `plans/[id]/edit/page.tsx` | parallel GET `/api/admin/plans/[id]` + GET `/api/admin/workouts` + GET `/api/admin/recipes` | POST `/api/admin/plans/[id]/days` (Save All Days), PUT `/api/admin/plans/[id]` (onBlur name/type/weeks) | 7-day grid editor with cell modal, "Copy to..." day cloning, per-week "Copy to all weeks". Local Map cache until Save All Days. |
...
| `/admin/workouts/new` | `workouts/new/page.tsx` | GET `/api/admin/workouts/categories` | POST `/api/admin/workouts` | `targetGoal === "All"` sent as null. |
| `/admin/workouts/[id]/edit` | `workouts/[id]/edit/page.tsx` | parallel GET `/api/admin/workouts/[id]` + categories | PUT `/api/admin/workouts/[id]` | `use(params)`. |
...
| `seed-coach-defaults.ts` | `seedCoachDefaults(coachId)`. Seeds 10 RecipeCategories, 12 DietaryTags, 5 WorkoutCategories + 28 WorkoutSubcategories, 85+ FoodItems. | `/api/super-admin/coaches` POST; `/api/super-admin/coaches/[id]` PUT action=`seedDefaults`; `scripts/backfill-coach-defaults.ts`. |
...
`prisma/seed.ts` (1807 lines, excluded from typecheck by `tsconfig.json:33`) is the modern equivalent: creates `admin@levelup.com / Lev3lUp@Adm!n2026`, `demo@levelup.com / user1234`, and 5 demo clients (`sarah@demo.com`, `james@demo.com`, `priya@demo.com`, `liam@demo.com`, `aoife@demo.com` — all `demo1234`) plus 3 months of data per user. Note that this seed also predates multi-tenant — it creates users with `role:"ADMIN"` and no coachId. Whether it's currently runnable depends on whether the schema's removed unique constraints on slug have been re-added.
...
Queried 2026-05-20. Passwords below come from the scripts that created them — there is no way to recover unknown hashes.

#### SUPER_ADMIN (1)

| Email | Password | Created by | Notes |
|---|---|---|---|
| `superadmin@fcma.com` | `SuperAdmin2026!` | `scripts/migrate-multi-tenant.ts:74` | Sole SUPER_ADMIN. |

#### COACHes (6)

| Email | Invite code | Likely password | Active clients | Notes |
|---|---|---|---|---|
| `admin@levelup.com` | `coach-danzel` | `Lev3lUp@Adm!n2026` if created via `prisma/seed.ts:34` | 2 (Emma, Jake) | Original ADMIN row, flipped to COACH by migrate-multi-tenant. Username "Coach Danzel". |
| `raheel.nabi@fcma.com` | `coach-flc4` | unknown — created live via `/super-admin/coaches` | 1 | Coach Raheel. |
| `raheel@fcma.com` | `raheel-78t6` | unknown | 0 | Raheel Nabi. |
| `john@mail.com` | `john-gckg` | unknown | 1 | John Smith. |
| `maaz@fcma.com` | `m-w` | unknown | 3 | Maaz Ahmed. Has the shortest invite code — likely manually edited. |
| `sarah@testcoach.com` | `sarah-b5ej` | unknown | 0 | Sarah Johnson (likely a test fixture). |

To reset a coach's password, a SUPER_ADMIN currently has **no UI for it** — only `/api/admin/users/[id]/password` exists, scoped to a coach updating their own USER's password. A SUPER_ADMIN would need to update via SQL or add a route.

#### USERs (11 — 4 inactive, 7 active)

| Email | Coach | planStatus | isActive | Notes |
|---|---|---|---|---|
| `emma@demo.com` | admin@levelup.com | ACTIVE | true | "Emma Sullivan", demo overperformer. Password `demo1234` per `scripts/seed-demo-data.ts:66` / `scripts/fix-emma.ts:33`. |
| `jake@demo.com` | admin@levelup.com | ACTIVE | true | "Jake Mitchell", demo underperformer. Password `demo1234`. |
| `daniyalarif264@gmail.com` | raheel@fcma.com | ACTIVE | false | Real user, deactivated. |
| `mahdiashfaq751@gmail.com` | raheel@fcma.com | ACTIVE | false | Real user, deactivated. |
| `raheelnabi27@gmail.com` | raheel@fcma.com | ACTIVE | false | Real user, deactivated. |
| `raheel.prada@gmail.com` | raheel.nabi@fcma.com | ACTIVE | true | Real user. |
| `lisa.sarah.client@example.com` | sarah@testcoach.com | PENDING | true | Test signup awaiting approval. |
| `maaz@gmail.com` | john@mail.com | ACTIVE | true | Real user under John. |
| `user_one@fcma.com` | maaz@fcma.com | ACTIVE | true | Test user. |
| `user_two@fcma.com` | maaz@fcma.com | ACTIVE | true | Test user. |
| `user_three@fcma.com` | maaz@fcma.com | ACTIVE | true | Test user. |

### Seed-only accounts (not in live DB)

These are referenced in `prisma/seed.ts` but not currently present in the production DB:
- `demo@levelup.com / user1234` (USER, role from seed; would conflict with multi-tenant now).
- `sarah@demo.com`, `james@demo.com`, `priya@demo.com`, `liam@demo.com`, `aoife@demo.com` — all `demo1234`, USERs created with 3 months of generated data by the seed's bottom half.

To run a local copy of the app with demo data, you'd need a fresh DB and a seed that respects multi-tenant — neither `prisma/seed.ts` nor `scripts/seed-database.ts` set `coachId`, so they won't produce a coherent multi-tenant state.
...
| 5 | med | `PlanDayMeal.templateDayId` and `clientDayId` are both nullable, no DB-level XOR | Could end up with rows attached to both or neither; nothing enforces it | `prisma/schema.prisma:591-607` | Add a CHECK constraint via a migration: `CHECK ((templateDayId IS NULL) <> (clientDayId IS NULL))`. |
...
| 23 | med | Plan-day editor accepts cross-coach `workoutId` / `recipeId` / `subcategoryId` from the body | Coach A can reference Coach B's content; later reads will show those names on Coach A's plans | `/api/admin/plans/[id]/days`, `/api/admin/plans/assign`, `/api/admin/workouts*` | Validate each FK with a `findFirst({where:{id, coachId}})` before insert. |
...
| 38 | low | `/admin/workouts` publish-toggle PUT sends `instructions:[]` and `subcategoryId:0` | Clobbers content if the PUT goes through | `AdminWorkoutsClient.tsx` | Send only `{isPublished}` or use a PATCH endpoint. |
