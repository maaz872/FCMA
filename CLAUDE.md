...
- Plans: `PlanTemplate → PlanTemplateDay → PlanDayMeal`; assigning to a user duplicates as `ClientPlan → ClientPlanDay`. `PlanDayMeal` has both `templateDayId?` and `clientDayId?` — only one is meant to be set (no DB-level XOR).
...
- **`PlanDayMeal.templateDayId` and `clientDayId`** are both nullable — only one is meant to be set at a time. No DB constraint enforces it.
