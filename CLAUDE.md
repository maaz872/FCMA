...
- **Per-coach seed library.** Every new coach gets 50 recipes + 50 workouts (with gif illustrations + bodyPart/equipment metadata) + 6 plan templates pre-wired via `seedCoachDefaults` (`src/lib/seed-coach-defaults.ts`). The curated content lives in `src/lib/seed/{recipes,workout-picks,plans}.ts`. Existing coaches opt in via `npx tsx scripts/backfill-coach-seed-content.ts <coachId>`.
- **Free Exercise DB is bundled.** `src/data/exercise-library.json` ships ~1 MB / 873 entries (MIT licensed). Query it via `searchExerciseLibrary` / `getExerciseLibraryEntry` in `src/lib/exercise-library.ts`. The coach-only proxy `/api/admin/exercise-library` keeps the JSON server-side so the IllustrationPicker doesn't bloat the client bundle.
- **Illustration picker components.** `src/components/admin/IllustrationPicker.tsx` (modal over the bundled library — picks a library entry, used by `/admin/workouts/new` and `/admin/workouts/[id]/edit` to attach gif + auto-fill metadata) and `src/components/admin/WorkoutPickerModal.tsx` (modal over the coach's existing `Workout` rows — used by `/admin/plans/[id]/edit` for the multi-exercise day editor).
